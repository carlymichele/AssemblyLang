...
int main()
{
	...
	cout<<"hexadecimal 1CA0 * D4482 = "<<multhex("1CA0", "D4482")<<endl<<endl;  //you should get B0076EC900

	cout<<"Sample Run 5" << endl;
	cout<<"binary 1 * 1 = "<<multbin("1", "1")<<endl;   //you should get 1
	cout<<"hexadecimal 4B * 5D = "<<multhex("4B", "5D")<<endl<<endl;  //you should get 21C9
...
string multbin(string bin1, string bin2)
{
	...
	if(bin1 == "0" || bin2 == "0"){ //2. Base Case: if either number is 0, the product is 0.
		binProd.append("0");
	}else if(bin1 == "1"){ //3. Base Case: if either number is 1, the product is equal to the other number.
		binProd.append(bin2);
	}else if(bin2 == "1"){
		binProd.append(bin1);
...
string multhex(string hex1, string hex2)
{
	...
	if(hex1 == "0" || hex2 == "0"){
		...
		// 8. mult is incremented for each bit value by the digit/letter.
		for(int bit = 0; bit < hex2.size(); bit++){
			//9. This ternary operator calculated the number of times to add by calculating the bit value and power of each bit position in the second hex number.
			mult += (pow(16, hex2.size() - bit - 1) * (hex2.at(bit) <= 57 ? hex2.at(bit) - 48 : hex2.at(bit) - 55));
		}

		for(int i = 0; i < mult; i++){ //10. Using the "mult" calculated above, the hexProd is added to itself n times.
			hexProd = addhex(hex1, hexProd);
		}
	}else{ /*11. If hex2 is larger than hex1, use hex1 as multiplier*/
		for(int bit = 0; bit < hex1.size(); bit++){
			mult += (pow(16, hex1.size() - bit - 1) * (hex1.at(bit) <= 57 ? hex1.at(bit) - 48 : hex1.at(bit) - 55));
		}
